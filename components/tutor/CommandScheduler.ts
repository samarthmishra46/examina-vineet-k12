...
export class CommandScheduler {
  ...
  private async playNarrate(cmd: NarrateCommand): Promise<void> {
    // Avatar path: delegate to the integrated voice+avatar provider when
    // the parent says the avatar is ready. If the avatar promise rejects,
    // fall through to the local TTS path so the lesson keeps playing.
    const useAvatar =
      this.deps.sayViaAvatar && this.deps.shouldRouteAudioLocally?.() === false;
    if (useAvatar && this.deps.sayViaAvatar) {
      try {
        // Provide a manual-skip hook so the skip button works during avatar
        // playback even though there's no BufferSource.
        const result = this.runWithSkip(this.deps.sayViaAvatar(cmd.text));
        await result;
        return;
      } catch (err) {
        console.warn('[lesson] avatar say failed, falling back to local TTS:', err);
